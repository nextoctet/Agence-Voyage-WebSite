...
export default async function DetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const offre = OFFRES.find((o: Offre) => o.id === id);
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 h-[550px] mb-12">
          <div className="md:col-span-2 overflow-hidden rounded-[2.5rem] shadow-lg">
            <Image src={offre.images[0]} width={400} height={300} className="w-full h-full object-cover hover:scale-105 transition duration-1000" alt={offre.titre} />
          </div>
          <div className="grid grid-rows-2 gap-4">
            <div className="rounded-[2.5rem] overflow-hidden shadow-md">
                <Image src={offre.images[1]} width={400} height={300} className="w-full h-full object-cover hover:scale-110 transition duration-700" alt={offre.titre} />
            </div>
            <div className="rounded-[2.5rem] overflow-hidden shadow-md">
                <Image src={offre.images[2]} width={400} height={300} className="w-full h-full object-cover hover:scale-110 transition duration-700" alt={offre.titre} />
            </div>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
              <div className="bg-blue-50/50 p-10 rounded-[2.5rem] border border-blue-100">
                ...
                <ul className="space-y-4">
                  {offre.details.inclus.map((x: string, i: number) => (
